...
/// <typeparam name="T"></typeparam>
/// <typeparam name="Error"></typeparam>
type ProtectedResultStateFunc<'State, 'T, 'Error> =
...
/// <typeparam name="T"></typeparam>
/// <typeparam name="Error"></typeparam>
type ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
...
/// <typeparam name="T"></typeparam>
/// <typeparam name="Error"></typeparam>
type AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
...
/// </summary>
[<Sealed>]
type ProtectedResultStateBuilder () =
    // 'T -> M<'T>
    member __.Return value
        : ProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
        Ok (value, state)
    ...
    member inline this.Zero ()
        : ProtectedResultStateFunc<'State, unit, 'Error> =
        this.Return ()

    // (unit -> M<'T>) -> M<'T>
    member __.Delay (generator : unit -> ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state -> generator () state

    // M<'T> * ('T -> M<'U>) -> M<'U>
    member __.Bind (m : ProtectedResultStateFunc<_,_,_>, binder : 'T -> ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, 'U, 'Error> =
        fun state ->
        match m state with
        | Error error ->
            Error error
    ...
    // or
    // M<unit> -> M<'T> -> M<'T>
    member this.Combine (r1 : ProtectedResultStateFunc<_,_,_>, r2 : ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, 'T, 'Error> =
        this.Bind (r1, (fun () -> r2))

    // M<'T> * (exn -> M<'T>) -> M<'T>
    member __.TryWith (body : ProtectedResultStateFunc<_,_,_>, handler : exn -> ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
        try body state
        with ex ->
    ...
    // M<'T> * (unit -> unit) -> M<'T>
    member __.TryFinally (body : ProtectedResultStateFunc<_,_,_>, handler)
        : ProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
        try body state
        finally
    ...
    // 'T * ('T -> M<'U>) -> M<'U> when 'T :> IDisposable
    member this.Using (resource : ('T :> System.IDisposable), body : 'T -> ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, 'U, 'Error> =
        fun state ->
        try
            body resource state
    ...
    // (unit -> bool) * M<'T> -> M<'T>
    member this.While (guard, body : ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, unit, 'Error> =
        if guard () then
            this.Bind (body, (fun () -> this.While (guard, body)))
        else
    ...
    // or
    // seq<'T> * ('T -> M<'U>) -> seq<M<'U>>
    member this.For (sequence : seq<_>, body : 'T -> ProtectedResultStateFunc<_,_,_>)
        : ProtectedResultStateFunc<'State, unit, 'Error> =
        this.Using (sequence.GetEnumerator (), fun enum ->
            this.While (
                enum.MoveNext,
...
type ReaderProtectedResultStateBuilder () =
    // 'T -> M<'T>
    member __.Return value
        : ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
        fun _ state ->
        Ok (value, state)
    ...
    // unit -> M<'T>
    member this.Zero ()
        : ReaderProtectedResultStateFunc<'Env, 'State, unit, 'Error> =
        fun _ state ->
        Ok ((), state)

    // (unit -> M<'T>) -> M<'T>
    member this.Delay (f : unit -> ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
        fun env state -> f () env state

    // M<'T> * ('T -> M<'U>) -> M<'U>
    member __.Bind (m : ReaderProtectedResultStateFunc<_,_,_,_>, binder : 'T -> ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'U, 'Error> =
        fun env state ->
    ...
    // or
    // M<unit> -> M<'T> -> M<'T>
    member this.Combine (r1 : ReaderProtectedResultStateFunc<_,_,_,_>, r2 : ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
        this.Bind (r1, (fun () -> r2))

    // M<'T> * (exn -> M<'T>) -> M<'T>
    member __.TryWith (body : ReaderProtectedResultStateFunc<_,_,_,_>, handler : exn -> ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
        fun env state ->
        try body env state
        with ex ->
    ...
    // M<'T> * (unit -> unit) -> M<'T>
    member __.TryFinally (body : ReaderProtectedResultStateFunc<_,_,_,_>, handler)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'T, 'Error> =
        fun env state ->
        try body env state
        finally
    ...
    // 'T * ('T -> M<'U>) -> M<'U> when 'T :> IDisposable
    member this.Using (resource : ('T :> System.IDisposable), body : 'T -> ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, 'U, 'Error> =
        fun env state ->
        try
    ...
    // (unit -> bool) * M<'T> -> M<'T>
    member this.While (guard, body : ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, unit, 'Error> =
        if guard () then
            this.Bind (body, (fun () -> this.While (guard, body)))
        else
    ...
    // or
    // seq<'T> * ('T -> M<'U>) -> seq<M<'U>>
    member this.For (sequence : seq<_>, body : 'T -> ReaderProtectedResultStateFunc<_,_,_,_>)
        : ReaderProtectedResultStateFunc<'Env, 'State, unit, 'Error> =
        this.Using (sequence.GetEnumerator (), fun enum ->
            this.While (
                enum.MoveNext,
...
/// </summary>
[<Sealed>]
type AsyncProtectedResultStateBuilder () =
    // 'T -> M<'T>
    member __.Return value
        : AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
            Ok (value, state)
            |> async.Return
    ...
    // unit -> M<'T>
    member inline this.Zero ()
        : AsyncProtectedResultStateFunc<'State, unit, 'Error> =
        this.Return ()

    // (unit -> M<'T>) -> M<'T>
    member this.Delay (generator : unit -> AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state -> generator () state

    // M<'T> * ('T -> M<'U>) -> M<'U>
    member __.Bind (m : AsyncProtectedResultStateFunc<_,_,_>, k : 'T -> AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, 'U, 'Error> =
        fun state ->
            async {
            let! result = m state
            match result with
    ...
    // or
    // M<unit> -> M<'T> -> M<'T>
    member this.Combine (r1 : AsyncProtectedResultStateFunc<_,_,_>, r2 : AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
        this.Bind (r1, (fun () -> r2))

    // M<'T> * (exn -> M<'T>) -> M<'T>
    member __.TryWith (body : AsyncProtectedResultStateFunc<_,_,_>, handler : exn -> AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
            async.TryWith (
    ...
    // M<'T> * (unit -> unit) -> M<'T>
    member __.TryFinally (body : AsyncProtectedResultStateFunc<_,_,_>, handler)
        : AsyncProtectedResultStateFunc<'State, 'T, 'Error> =
        fun state ->
            async.TryFinally (
                async.Delay (fun () -> body state),
    ...
    // 'T * ('T -> M<'U>) -> M<'U> when 'T :> IDisposable
    member this.Using (resource : ('T :> System.IDisposable), body : 'T -> AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, 'U, 'Error> =
        this.TryFinally (
            this.Delay (fun () ->
                body resource),
    ...
    // (unit -> bool) * M<'T> -> M<'T>
    member this.While (guard, body : AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, unit, 'Error> =
        if guard () then
            this.Bind (body, (fun () -> this.While (guard, body)))
        else
    ...
    // or
    // seq<'T> * ('T -> M<'U>) -> seq<M<'U>>
    member this.For (sequence : seq<_>, body : 'T -> AsyncProtectedResultStateFunc<_,_,_>)
        : AsyncProtectedResultStateFunc<'State, unit, 'Error> =
        this.Using (sequence.GetEnumerator (), fun enum ->
            this.While (
                enum.MoveNext,
...
module ResultWorkflowBuilders =
    ...
    //
    [<CompiledName("ProtectedResultState")>]
    let protectedResultState = ProtectedResultStateBuilder ()
    //
    ...
    //
    [<CompiledName("AsyncProtectedResultState")>]
    let asyncProtectedResultState = AsyncProtectedResultStateBuilder ()
    //
...
module ProtectedResultState =
    ...
    let inline liftReader (readerFunc : ReaderFunc<'Env, 'T>)
        : ProtectedResultStateFunc<'Env, 'T, 'Error> =
        fun env ->
    ...
    let inline discardState (protectedResultStateFunc : ProtectedResultStateFunc<'State, 'T, 'Error>) =
        fun state ->
